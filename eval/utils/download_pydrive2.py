...
def authenticate_with_google_drive(
    ...
    Set force_reauth=True to bypass cached credentials and perform a fresh login.
    """
    gauth = GoogleAuth()
    ...
    if not force_reauth and os.path.exists(credentials_path):
        gauth.LoadCredentialsFile(credentials_path)

    # Ensure offline access; save explicitly to avoid backend config error
...
def main(folder_id, target_dir, redownload, client_secret_pth, credentials_pth, file_spec=None, reauth=False):
    """
    Main workflow:
    ...
    drive = authenticate_with_google_drive(client_secret_pth, credentials_pth, force_reauth=reauth)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--reauth", action="store_true", default=False,
                        help="Force re-authentication and bypass cached credentials.")

    args = parser.parse_args()
    main(args.folder_id, args.target_dir, args.redownload, args.client_secret_pth, None, args.file, args.reauth)
