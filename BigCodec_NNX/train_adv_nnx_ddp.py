...
from data_grain import LibriTTSDataset
import grain
...
def train_adv(
    ...
    gen_tx = optax.chain(
        ...
        optax.adamw(gen_lr_schedule, b1=0.8, b2=0.9, weight_decay=weight_decay)
    )
    disc_tx = optax.chain(
        optax.clip_by_global_norm(disc_grad_clip),
        optax.adamw(disc_lr_schedule, b1=0.8, b2=0.9, weight_decay=weight_decay)
    )
    ...
    path = ocp.test_utils.erase_and_create_empty('/home/hoyeol/BigCodec_NNX/bigcodec_nnx_checkpoints/')
    current_state = sharded_state
    for epoch in range(1, epochs+1):
        for batch in train_loader:
            ...
            # print(batch[0].shape)
            batch = np.transpose(batch[0], (0, 2, 1))
            # print(jnp.std(batch[0]))
            current_state, metrics = train_step(current_state, batch)

            step += 1
            if step % steps_per_epoch == 0 or step >= train_steps: # Check if train_steps is used
                break
            ...
            if step % 10 == 0:
                ...
                avg_steps_per_sec = step / (current_time - start_time) if (current_time - start_time) > 0 else 0
        ...
        checkpointer.save(f'{path}/epoch_{epoch}_step_{step}', current_state)
    ...
    avg_steps_per_sec = step / total_time if total_time > 0 else 0
    if jax.process_index() == 0:
        print('\n훈련 완료:\n총 스텝: {}\n총 소요 시간: {:.2f}초\n평균 스텝/초: {:.2f}'.format(
            step, float(total_time), float(avg_steps_per_sec)))
...
def main():
    ...
    mp_policy = jmp.get_policy("params=float32,compute=float32,output=float32")
    ...
    def preprocess_audio(data):
        ...
        # Assuming the NNX modules were adapted to expect (B, T, C) based on the transpose here.
        # If they expect (B, C, T), remove the transpose. Let's keep it for now.
        return np.transpose(audio_data, (1, 0))
    ...
    if jax.process_index() == 0:
        print(f"Using {n_workers} workers for DataLoader")
    train_loader = grain.DataLoader(
        data_source=train_dataset,
        operations=[grain.transforms.Batch(total_batch_size, drop_remainder=True)],
    ...
    # train_loader = grain.MapDataset.source(train_dataset).map(preprocess_audio).shuffle(seed=42).batch(total_batch_size, drop_remainder=True)
