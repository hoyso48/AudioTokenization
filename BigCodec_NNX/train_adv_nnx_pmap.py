...
# Assume these are now NNX-based modules
# Keep TrainState for now, but might need adjustment or replacement later
...
def train_adv(
    ...
    # NNX model uses float32 for params by default unless dtype specified in layers
    model = CodecModule(rngs=nnx.Rngs(model_key))
    ...
    if jax.process_index() == 0:
        print("Initial Generator State Keys:", list(gen_params.keys()))
        print("Initial Discriminator State Keys:", list(disc_params.keys()))
        # Verify the split worked as expected. If keys are empty, the filters need adjustment.
        if not gen_params or not disc_params:
             print("Warning: Parameter split might be incorrect. Check NNX filters.")
    ...
    # Adapt TrainState to hold NNX State parts and GraphDef
    # Option 1: Keep Linen TrainState (requires careful state management)
    # Option 2: Create a new NNX-specific state holder (potentially cleaner)

    # Let's stick to adapting Linen's TrainState for minimal change request
    ...
    initial_state_obj = AdvTrainState(
        step=0,
        graphdef=graphdef,
    ...
    def train_step_pmap(state, inputs):
        # 1. 판별자 업데이트
    ...
    for epoch in range(1, epochs+1):
        ...
        for batch in train_loader:
            ...
            if step % 10 == 0:
                ...
                if jax.process_index() == 0:
                    loss_str = ", ".join([f"{k}={float(v[0]):.4f}" for k, v in metrics.items() if 'loss' in k])
                    lr_str = f"gen_lr={float(metrics['gen_lr'][0]):.1E}, disc_lr={float(metrics['disc_lr'][0]):.1E}"
        ...
        if jax.process_index() == 0:
            checkpointer.save(f'/home/hoyeol/Audio_Tokenization/BigCodec_NNX/bigcodec_nnx_checkpoints/state_{epoch}_{total_steps}', current_state)
    ...
    # 최종 파라미터를 가져오기 위해 State 언랩핑
    # We need to merge the final states back if we want a final model object
    # Or just return the state components
    final_state_host = jax.device_get(jax.tree_util.tree_map(lambda x: x[0], current_state))

    # Optionally merge back into a model object on host
    # final_model = nnx.merge(final_state_host.graphdef,
    #                         final_state_host.gen_params,
    #                         final_state_host.disc_params) #, final_state_host.other_state)

    # Return the raw state components for potential saving
    return final_state_host
...
def main():
    ...
    # TODO: Save the final_gen_state and final_disc_state using Orbax
    if jax.process_index() == 0:
        print("Training finished. Final states obtained.")
        # Example: Save using Orbax (requires graphdef potentially)
        # checkpointer = ocp.StandardCheckpointer()
        # graphdef = ... # Need the graphdef from train_adv return or re-split
        # checkpointer.save(f'/tmp/final_gen_state', final_gen_state)
        # checkpointer.save(f'/tmp/final_disc_state', final_disc_state)
